...
void set_neo_LB(uint32_t c=0){
  pixels.fill(c, 0, 6);
}

void set_neo_LF(uint32_t c=0){
  pixels.fill(c, 18, 6);
}

void set_neo_RF(uint32_t c=0){
  pixels.fill(c, 36, 6);
}

void set_neo_RB(uint32_t c=0){
  pixels.fill(c, 54, 6);
}

void set_neo_Right(uint32_t c=0){
  pixels.fill(c, 6, 12);
}

void set_neo_Left(uint32_t c=0){
  pixels.fill(c, 42, 12);
}
...
void loop() {
  ...
  if(millis() - lastLEDTime > 100){
     static int hue_val = 43000;
     static int sat_val = 0;
     static int val_val = 100;
     static int dt_sat = 10;
     if(sat_val > 240){
      dt_sat = -10;
     }
     else if(sat_val < 20){
     ...
     sat_val = sat_val + dt_sat;
     pixels.setPixelColor(indexLED, pixels.Color(0, 0, 0));
     indexLED = (indexLED+1)%NEOPIXEL_NUM;
     set_neo_LF(pixels.Color(150, 0, 0));
     set_neo_LB(pixels.Color(150, 0, 0));
     set_neo_RF(pixels.Color(150, 0, 0));
     set_neo_RB(pixels.Color(150, 0, 0));
     
     uint32_t rgbcolor = pixels.ColorHSV(hue_val, sat_val, val_val);
     /*if(pixels.getPixelColor(6) == pixels.Color(80, 80, 80)){
        set_neo_Left(pixels.Color(0, 0, 80));
        set_neo_Right(pixels.Color(0, 0, 80));
     }
     else{
        set_neo_Left(pixels.Color(80, 80, 80));
        set_neo_Right(pixels.Color(80, 80, 80));
     }*/
     set_neo_Left(rgbcolor);
     set_neo_Right(rgbcolor);
     //pixels.setPixelColor(indexLED, pixels.Color(150, 0, 0));
     pixels.show();
     lastLEDTime = millis();
  }
